...
Write-Host ""
Write-Host ""
Write-Host "1. Change the file names, folder names, and project references from the ShogunEra to your custom solution name."
Write-Host ""
...
Move-Item -Path ..\$Name\ShogunEra -Destination ..\$Name\$Name
$unity = $Name + "Unity"
Move-Item -Path ..\$Name\JotunnModUnity -Destination ..\$Name\$unity
Get-ChildItem -Path ..\$Name\ -File -Recurse | % -Process{if($_.Name -ne "ShogunEra.zip") {Rename-Item -Path $_.PSPath -NewName $_.Name.replace("ShogunEra", $Name)}}
}
else{
...
$msg = "     . . . Replacing internal references to 'ShogunEra' with " + '($Name)' + " . . ."
Write-Host $msg
((Get-Content -path ..\$Name\$Name.sln -Raw) -replace 'ShogunEra',$Name) | Set-Content -Path ..\$Name\$Name.sln
((Get-Content -path ..\$Name\$Name\$Name.cs -Raw) -replace 'ShogunEra',$Name) | Set-Content -Path ..\$Name\$Name\$Name.cs 
...
((Get-Content -path ..\$Name\$Name\$Name.csproj -Raw) -replace 'ShogunEra',$Name) | Set-Content -Path ..\$Name\$Name\$Name.csproj
((Get-Content -path ..\$Name\$Name\Properties\AssemblyInfo.cs -Raw) -replace 'ShogunEra',$Name) | Set-Content -Path ..\$Name\$Name\Properties\AssemblyInfo.cs
...
if (Test-Path -Path ..\Environment.props) {	
    ...
    $yn = Read-Host "Step 3. . . . An Environment.Props file is dectected in the parent directory. Copy this file to solution directory (y/n)?"
	 if ($yn -eq "y") {
		...
		Write-Host "https://github.com/Valheim-Modding/ShogunEra"
		Write-Host ""
		Write-Host ""
